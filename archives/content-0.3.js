// ChatGPT Export Extension v2.1 - Simplified & Focused
// Timeline-based range selection, streamlined UI, proper state management
...
class ExportState {
  ...
  setFilter(filterName, value) {
    if (filterName === 'fullChat' && value) {
      // Full Chat disables all other filters
      this.filters = {
        fullChat: true,
        assistantOnly: false,
        code: false,
    ...
    } else if (filterName !== 'fullChat') {
      // Any other filter disables Full Chat
      this.filters.fullChat = false;
      this.filters[filterName] = value;
    }
  }

  getActiveFilters() {
    if (this.filters.fullChat) return { all: true };
    return Object.entries(this.filters)
      .filter(([key, value]) => key !== 'fullChat' && value)
      ...
      <div class="panel" role="dialog" aria-label="Export ChatGPT Conversation" aria-modal="true">
        <div class="header">
          ...
          <div class="filter-buttons">
            <button class="filter-btn active" data-filter="fullChat">
              <span class="icon">📄</span> Full Chat
            </button>
        ...
        <div class="footer">
          ...
          <button class="secondary-btn" data-action="clear-range">
            Clear Selection
          </button>
  ...
  attachEventHandlers(shadow, harvest) {
    ...
    filterBtns.forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        if (filter === 'fullChat') {
          // Full Chat disables all others
          globalState.setFilter('fullChat', true);
          ...
          shadow.querySelector('[data-filter="fullChat"]').classList.remove('active');
        }
...
const MessageFilter = {
  apply(messages) {
    ...
    const activeFilters = globalState.getActiveFilters();
    
    if (!activeFilters.all) {
      filtered = filtered.filter(msg => {
        // Role filter
        if (activeFilters.assistantOnly && msg.role !== 'assistant') {
          return false;
        }
        
        // Content filters
        if (activeFilters.code || activeFilters.tables || activeFilters.lists) {
          const hasCode = msg.blocks?.some(b => b.kind === 'code');
          const hasTables = msg.blocks?.some(b => b.kind === 'table');
          const hasLists = msg.blocks?.some(b => b.kind === 'list');
          
          return (activeFilters.code && hasCode) ||
                 (activeFilters.tables && hasTables) ||
                 (activeFilters.lists && hasLists);
        }
