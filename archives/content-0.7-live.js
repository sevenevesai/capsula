// ChatGPT Export Extension v2.4 - Fixed timeline alignment for incomplete assistant messages
// Fixed: Properly handles thinking states, failed responses, and legitimate duplicate messages
...
const CFG = {
  ...
  theme: {
    light: {
      ...
      thinkingTimeline: '#fbbf24', // New color for thinking states
    ...
    dark: {
      ...
      thinkingTimeline: '#f59e0b', // New color for thinking states
      accentPrimary: '#3b82f6',
      accentSecondary: '#10b981',
      exportButton: '#3b82f6',
...
const Timeline = {
    ...
    messages.forEach((msg, idx) => {
      ...
      timeline += `
        <div class="timeline-segment" 
             ...
             title="${msg.role === 'user' ? 'You' : msg.isThinking ? 'Thinking...' : 'ChatGPT'}">
        </div>
      `;
    });
  ...
  setupScrollIndicator(container) {
    ...
    const updateIndicator = () => {
      const scrollPercent = previewPanel.scrollTop / 
        (previewPanel.scrollHeight - previewPanel.clientHeight);
      ...
      const indicatorTop = scrollPercent * (containerHeight - 20);
...
const ExportPanel = {
  ...
  getStyles(colors) {
    return `
      ...
      .timeline-segment[data-thinking="true"]::after {
        content: "⚡";
        position: absolute;
        right: 2px;
        top: 50%;
      ...
      .message.thinking .bubble {
        background: ${colors.thinkingTimeline};
        color: ${colors.text};
        opacity: 0.8;
      }
  ...
  renderMessages(container, harvest) {
    ...
    filteredMessages.forEach(msg => {
      ...
      messageDiv.className = `message ${msg.role}${msg.isThinking ? ' thinking' : ''}`;
      ...
      messageDiv.innerHTML = `
        <div class="bubble">
          <div class="message-role">${msg.role === 'user' ? 'You' : msg.isThinking ? 'ChatGPT (Thinking)' : 'ChatGPT'}</div>
          ${bubbleContent}
        </div>
      `;
...
const MessageFormatter = {
  format(message) {
    if (message.isThinking) {
      return `<p style="opacity: 0.8; font-style: italic;">${Utils.escapeHtml(message.plain?.text || 'Thinking...')}</p>`;
    }
...
const ExportManager = {
  ...
  toMarkdown(messages, meta) {
    ...
    messages.forEach(msg => {
      const roleLabel = msg.role === 'user' ? 'You' : msg.isThinking ? 'ChatGPT (Thinking)' : 'ChatGPT';
      md += `## ${roleLabel}\n\n`;
      
      if (msg.isThinking) {
        md += `*${msg.plain?.text || 'Thinking...'}*\n\n`;
      } else {
...
<head>
  ...
  <style>
    ...
    .message.thinking { opacity: 0.7; font-style: italic; }
    .bubble {
      max-width: 70%;
      padding: 12px 16px;
      border-radius: 16px;
...
<body>
  ...
  ${messages.map(msg => `
    <div class="message ${msg.role}${msg.isThinking ? ' thinking' : ''}">
      <div class="bubble">
        <div class="role">${msg.role === 'user' ? 'You' : msg.isThinking ? 'ChatGPT (Thinking)' : 'ChatGPT'}</div>
...
const Harvester = {
  ...
  collectAllMessages() {
    ...
    turnContainers.forEach((container, index) => {
      ...
      const isThinking = this.isThinkingMessage(container);
      ...
      const message = {
        ...
        incomplete: isThinking, // Mark thinking messages as incomplete
        blocks: blocks.length ? blocks : (plainText ? [{ kind: 'para', md: plainText }] : []),
        plain: { text: plainText || (isThinking ? this.getThinkingText(container) : '') },
        timestamp: new Date().toISOString()
      };
      
      // Only add if there's actual content or it's a thinking state
      if (message.plain.text || message.blocks.length > 0 || isThinking) {
  ...
  findAllTurnContainers() {
    ...
    const strategies = [
      ...
      () => {
        const elements = [];
        // Find elements containing thinking text patterns
        const thinkingPatterns = [
          'Thought for', 'Thinking for', 'Analyzing', 'Processing',
          'Stopped', 'analyzing', 'seconds', 'secs'
        ];
        
        document.querySelectorAll('div, article').forEach(el => {
          const text = el.textContent || '';
          if (thinkingPatterns.some(pattern => text.includes(pattern))) {
  ...
  isThinkingMessage(el) {
    ...
    // Check for thinking indicators in text
    const hasThinkingText = thinkingIndicators.some(indicator => text.includes(indicator));
    ...
    // Check if it's followed by the same user message (indicates retry)
    const nextSibling = el.nextElementSibling;
    if (nextSibling) {
      // ⚠️ IMPORTANT: Avoid recursion loop by skipping thinking checks when resolving roles here.
      const currentRole = this.detectRole(el, { skipThinking: true });
      const nextRole = this.detectRole(nextSibling, { skipThinking: true });
      if (currentRole === 'assistant' && nextRole === 'user') {
        const currentUserBefore = this.findPreviousUserMessage(el);
        if (currentUserBefore && nextSibling) {
          const beforeText = (currentUserBefore.textContent || '').trim();
          const afterText = (nextSibling.textContent || '').trim();
          if (beforeText && afterText && beforeText === afterText) {
            return true; // This assistant message didn't complete
          }
        }
      }
    }

    return hasThinkingText && hasExpandButton;
  },


  findPreviousUserMessage(el) {
    let current = el.previousElementSibling;
    while (current) {
      // ⚠️ Avoid recursion loop by skipping thinking checks while resolving roles.
      if (this.detectRole(current, { skipThinking: true }) === 'user') {
        return current;
      }
  ...
  getThinkingText(el) {
    // Extract meaningful thinking state text
    const patterns = [
      /Thought for [\d.]+ seconds?/i,
      /Thinking for [\d.]+ seconds?/i,
      /Stopped analyzing/i,
  ...
  detectRole(el, opts = {}) {
    ...
    // ✅ Prevent mutual recursion: only consult thinking state if not explicitly skipped
    if (!skipThinking && this.isThinkingMessage(el)) return 'assistant';
