/**
 * ChatGPT Export Extension v3.2.0 - Production Release
 * 
 * A Firefox extension for exporting ChatGPT conversations to various formats.
 * Features include message filtering, range selection, and multiple export formats.
 * 
 ...
 * - URLs are sanitized to prevent javascript: protocol attacks
 * - Exported HTML includes CSP headers to prevent script execution
 * - Uses Shadow DOM to isolate extension UI from page content
 * 
 ...
 * - Content script only, no background scripts needed
 */
...
const CFG = {
  ...
  navCheckIntervalMs: 3000, // Reduced from 500ms for better performance
  ...
  /**
   * Comprehensive patterns for detecting ChatGPT thinking states
   * These appear when models like o1-preview are processing
   */
  thinkingPatterns: {
    // Time-based patterns (e.g., "Thought for 15 seconds")
    timePatterns: [
      /(?:Thought|Thinking|Processing|Analyzing|Working) for (\d+(?:\.\d+)?)\s*(?:second|sec)s?/i,
      /(?:Thought|Thinking|Processing|Analyzing|Working) for (\d+)\s*(?:minute|min)s?(?:\s+(?:and\s+)?(\d+)\s*(?:second|sec)s?)?/i,
      /(?:Thought|Thinking) for (\d+)m\s*(\d+)s/i,
    ],
    // State patterns (e.g., "Analyzing...", "Analysis stopped")
    statePatterns: [
      /^(?:Analyzing|Analyzed|Processing|Searching|Thinking|Working|Calculating|Reasoning)\.{0,3}$/i,
      /^(?:Analysis|Processing) (?:stopped|paused|complete)\.{0,3}$/i,
      /^(?:Stopping|Resuming|Starting) (?:analysis|processing)\.{0,3}$/i,
      /^(?:Browsing|Looking up|Reading|Planning|Outlining|Drafting)\.{0,3}$/i,
      /^(?:Summarizing|Refining|Formatting|Compiling|Running|Executing)\.{0,3}$/i,
      /^(?:Uploading|Transcribing|Translating)\.{0,3}$/i,
      /^(?:Using tools|Tool call(?:ing)?|Calling tool)\.{0,3}$/i,
...
/**
 * Manages the global state of the export panel
 * Tracks filters, selection ranges, and export format
 */
class ExportState {
  constructor() {
    this.reset();
...
const ThemeUtils = {
  ...
  /**
   * Extract ChatGPT's font stack from the page
   * Falls back to known ChatGPT font stack if detection fails
   */
  getChatGPTFont() {
...
const ExportButton = {
  /**
   * Create the floating export button with shadow DOM
   * @param {Function} onClick - Callback when button is clicked
   * @returns {HTMLElement} The button host element
   */
  create(onClick) {
...
const Timeline = {
  /**
   * Render the vertical timeline showing all messages
   * Excludes thinking states from timeline to avoid clutter
   */
  render(container, messages) {
    ...
    realMessages.forEach((msg) => {
      ...
      const color = msg.role === 'user' ? colors.userTimeline : colors.assistantTimeline;
  ...
  /**
   * Setup scroll-based highlighting of current message
   */
  setupScrollHighlighting(container) {
    ...
    const updateHighlight = () => {
      ...
      // Find which message is at viewport center
      ...
      segments.forEach((segment) => {
        ...
        if (segIndex === currentIndex && isInRange) {
          ...
          segment.style.boxShadow = `
            inset 0 0 0 2px ${ThemeUtils.getColors().accentPrimary}, 
            0 0 0 2px ${ThemeUtils.getColors().accentPrimary}, 
  ...
  /**
   * Attach interaction handlers for timeline selection
   */
  attachHandlers(container) {
    ...
    track.addEventListener('mousedown', (e) => {
      ...
      if (segment) {
        ...
        // Clear selection if clicking outside current range
  ...
  updateSelection(container) {
    ...
    if (globalState.rangeSelection.start !== null && 
      ...
      // Find segments for start and end of range
  ...
  /**
   * Handle resize of selection range
   */
  ...
  /**
   * Check if message index is within selected range
   */
...
const ExportPanel = {
  /**
   * Create the main export panel with shadow DOM
   */
  create(harvest) {
    // Reset state on panel creation
    globalState.reset();
  ...
  /**
   * Get complete styles for the panel
   */
  ...
  /**
   * Render messages in the preview pane
   */
  renderMessages(container, harvest) {
    ...
    filteredMessages.forEach(msg => {
      ...
      // Handle thinking states as labels
      let thinkingLabel = '';
...
const MessageFilter = {
  /**
   * Apply active filters to message list
   */
  apply(messages) {
...
const MessageFormatter = {
  /**
   * Format message blocks into HTML
   */
  format(message) {
...
const ExportManager = {
  ...
  toMarkdown(messages, meta) {
    ...
    messages.forEach(msg => {
      ...
      // Only add header and content if there's actual content
      ...
      if (hasContent && !msg.isThinking) {
        md += `## ${roleLabel}\n\n`;
        if (hasBlocks) {
          md += Utils.blocksToMarkdown(msg.blocks) + '\n';
        } else if (hasPlain) {
          md += msg.plain.text + '\n\n';
        }
      }
  ...
  /**
   * Convert messages to HTML format with security headers
   */
...
<body>
  ...
  ${messages.map(msg => {
    // Handle thinking label
    const thinkingLabel = (msg.isThinking && msg.thinkingLabel)
      ? `<div class="thinking-label">${Utils.escapeHtml(msg.thinkingLabel)}</div>` : '';
    ...
    // Skip empty messages
    if (!hasBubbleContent && !thinkingLabel) return '';
...
const Harvester = {
  /**
   * Harvest all messages from the current conversation
   */
  async harvest() {
    const meta = {
  ...
  collectAllMessages() {
    ...
    // Find all conversation turns using multiple strategies
    ...
    turnContainers.forEach((container, index) => {
      ...
      // Add message if it has content or is a thinking state
  ...
  /**
   * Find all conversation turn containers using multiple strategies
   */
  findAllTurnContainers() {
    ...
    const strategies = [
      // Direct conversation turn elements
      () => document.querySelectorAll('[data-testid="conversation-turn"]'),
      
      // Elements with message author role
      () => document.querySelectorAll('[data-message-author-role]'),
      
      // Article elements used for messages
  ...
  /**
   * Detect if element contains a thinking state
   */
  detectThinkingState(el) {
    ...
    // Check for expand/collapse buttons with thinking keywords
  ...
  /**
   * Check if element has actual content (not just UI elements)
   */
  hasActualContent(el) {
  ...
  /**
   * Find the content node within a message container
   */
  findContentNode(el) {
    const candidates = [
  ...
  /**
   * Sanitize cloned content by removing UI elements
   */
  sanitizeClone(root) {
    const DROP = [
      'button',
...
const ContextMenu = {
  ...
  /**
   * Extract single message for context menu export
   */
...
const OverlayManager = {
  /**
   * Mount the export button overlay
   */
  mount() {
  ...
  /**
   * Unmount the export button overlay
   */
  unmount() {
    this.stopObservers();
  ...
  /**
   * Recompute button position to avoid overlapping with composer
   */
  recomputePosition() {
  ...
  /**
   * Find the message composer element
   */
  findComposer() {
    const candidates = [
  ...
  /**
   * Start observers for DOM changes and resize
   */
  startObservers() {
...
const App = {
  ...
  /**
   * Setup route watcher for SPA navigation
   */
  setupRouteWatcher() {
    ...
    // Override history methods for immediate detection
    ...
    // Low-frequency polling as a fallback for SPA nav quirks
...
// Ensure we clean up on page hide to avoid leaks across navigations
